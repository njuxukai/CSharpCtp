...
trader_api_export_skip_list = ['Init','Release','Join', 'RegisterSpi', 'SubscribePrivateTopic', 'SubscribePublicTopic', 'ReqUserLogin', 'ReqAuthenticate']
trader_spi_define_auto_gen_skip_list = ['OnFrontConnected', 'OnRspAuthenticate', 'OnRspUserLogin']

def parse_trader_file(src_path):
    lines = load_src_file(src_path + 'ThostFtdcTraderApi.h')
    api_function_specs = []
    spi_function_specs = []
    in_api_body = False
    in_spi_body = False
    for i in range(len(lines)):
        ...
        if is_remark_line(line):
            continue
        if line.find('class') != -1 and line.find('CThostFtdcTraderApi') != -1:
            in_api_body = True
            continue
        if line.find('class') != -1 and line.find('CThostFtdcTraderSpi') != -1:
            in_spi_body = True
            continue
        if line[0:2] == '};':
            in_api_body = False
...
def generate_c_header(dest_path, api_func_specs, spi_func_specs):
    template  = load_template_file('./template/cctp_trader.h.template')
    d = {}

    fp_define_lines = []
    line_template = 'typedef void(__cdecl *%s)(%s) ;'
    fp_name_template = 'FuncPtrTrade%s'
    ...
    d['trade_fp_types'] = add_whitespaces('\n'.join(fp_define_lines), 0)

    #trader_api_declare
    lines = []
    line_template = 'API_EXPORT %s %s(void* trader);'
    line_template2 = 'API_EXPORT %s %s(void* trader, %s);'
    skip_api_names = set(trader_api_export_skip_list)
    ...
    d['trader_api_declare'] = add_whitespaces('\n'.join(lines), 4)
        
    #trader_spi_register
    lines = []
    line_template = 'API_EXPORT void RegisterTradeFP%s(void* trader, FuncPtrTrade%s fp);'
    for spec in spi_func_specs:
        if spec.name in skip_api_names or spec.rtn_type_name == '':
            continue
        lines.append(line_template %(spec.name, spec.name))
    d['trader_spi_register'] = add_whitespaces('\n'.join(lines), 4)

    target_fpath = '{0}/cctp_trader.h'.format(dest_path) 
    save_cpp_file(template.format_map(d), target_fpath)


def generate_c_source(dest_path, api_func_specs, spi_func_specs):
    template  = load_template_file('./template/cctp_trader.cpp.template')
    d = {}
    ...
    target_fpath = '{0}/cctp_trader.cpp'.format(dest_path)
    save_cpp_file(template.format_map(d), target_fpath)
...
def _generate_c_api_call_function(spec, template):
    ...
    body_template ='''    CTrader* pTrader = (CTrader*)phandler;
    return pTrader->%s(%s);'''
...
def _generate_c_spi_register_function(spec, template):
    ...
    d['func_name'] =   'RegisterTradeFP%s' % spec.name
    parameters = 'void* phandler, FuncPtrTrade%s fp' % spec.name
    d['parameter_list'] = parameters
    body_template ='''    CTrader* pTrader = (CTrader*)phandler;
    pTrader->registerFP_%s(fp);'''
...
def generate_trader_header(dest_path, api_func_specs, spi_func_specs):
    template  = load_template_file('./template/CTrader.h.template')
    d = {}
    #api_function_signatures
    skip_func_names = set(trader_api_export_skip_list)
    ...
    lines = []
    line_template = 'void registerFP_%s(FuncPtrTrade%s fp);'
    for spec in spi_func_specs:
        lines.append(line_template %(spec.name, spec.name))
    ...
    line_template = '%s %s;'
    for spec in spi_func_specs:
        fp_type_name = 'FuncPtrTrade%s' % spec.name
        fp_name = 'm_fp%s' % spec.name
        lines.append(line_template % (fp_type_name, fp_name))
    d['function_pointers'] = add_whitespaces('\n'.join(lines), 4)


    target_fpath = '{0}/CTrader.h'.format(dest_path) 
    save_cpp_file(template.format_map(d), target_fpath)

def generate_trader_source(dest_path, api_func_specs, spi_func_specs):
    template  = load_template_file('./template/CTrader.cpp.template')
    d = {}
    ...
    target_fpath = '{0}/CTrader.cpp'.format(dest_path) 
    save_cpp_file(template.format_map(d), target_fpath)
...
def _generate_cpp_call_api_function(spec, template):
    d = {}
    d['rtn_type_name'] = spec.rtn_type_name
    d['func_name'] = 'CTrader::%s' % spec.name
...
def _generate_cpp_link_spi_to_callbacks(spi_func_specs):
    lines = []
    skip_func_names = []
    skip_func_names = set(trader_spi_define_auto_gen_skip_list)
    template  = load_template_file('./template/function.template')
    for spec in spi_func_specs:
...
def _generate_cpp_register_callback(spec, template):
    d = {}
    d['rtn_type_name'] = 'void'
    d['func_name'] = 'CTrader::registerFP_%s' % spec.name
    d['parameter_list'] = 'FuncPtrTrade%s fp' % spec.name
    body = '''m_fp%s = fp;
m_%sRegistered = true;'''
    lower_name = spec.name[0].lower() + spec.name[1:]
...
def generate_files(src_path, c_dest_path):
    api_function_specs, spi_function_specs = parse_trader_file(src_path)
    ...
    generate_trader_header(c_dest_path, api_function_specs, spi_function_specs)
    generate_trader_source(c_dest_path, api_function_specs, spi_function_specs)
