# -*- coding: utf-8 -*- 
from c_mduser_parser import parse_mduser_file
from ctp_file_util import *

api_skip_list = ['CreateFtdcMdApi', 'GetApiVersion', 'Release',
                ...
                '~CThostFtdcMdApi','SubscribeMarketData','UnSubscribeMarketData','SubscribeForQuoteRsp','UnSubscribeForQuoteRsp']

def generate_files(src_dir, dest_dir):
    api_specs, spi_specs = parse_mduser_file(src_dir)
    generate_spi_delegate_file(spi_specs, dest_dir)
    generate_spi_event_args(spi_specs, dest_dir)
    generate_mduser_dll_wrapper(api_specs, spi_specs, dest_dir)
    generate_mduser_wrapper(api_specs, spi_specs, dest_dir)



def generate_spi_delegate_file(spi_specs, dest_dir):
    template  = load_template_file('./template/MdUserSpiDelegates.cs.template')
    d = {}
    delegate_lines = []
    ...
    target_fpath = '{0}/MdUserSpiDelegates.cs'.format(dest_dir) 
    save_cpp_file(template.format_map(d), target_fpath)


def generate_spi_event_args(spi_specs, dest_dir):
    template  = load_template_file('./template/MdUserSpiEventArgs.cs.template')
    ...
    target_fpath = '{0}/MdUserSpiEventArgs.cs'.format(dest_dir) 
    save_cpp_file(template.format_map(d), target_fpath)


def generate_mduser_dll_wrapper(api_specs, spi_specs, dest_dir):
    template  = load_template_file('./template/MdUserDllWrapper.cs.template')
    d = {}
    ...
    target_fpath = '{0}/MdUserDllWrapper.cs'.format(dest_dir) 
    save_cpp_file(template.format_map(d), target_fpath)


def generate_mduser_wrapper(api_specs, spi_specs, dest_dir):
    template  = load_template_file('./template/MdUserWrapper.cs.template')
    d = {}
    #delegate_vars
    lines  = []
    for spec in spi_specs:
    ...
    lines = []
    fmt = 'MdUserDllWrapper.Register%sCallback(m_handler, %sDelegate);'
    for spec in spi_specs:
        node1 = spec.name
    ...
    for spec in api_specs:
        ...
        lines.append('')
    d['mduser_api_functions'] = add_whitespaces('\n'.join(lines) ,8)
    target_fpath = '{0}/MdUserWrapper.cs'.format(dest_dir) 
    save_cpp_file(template.format_map(d), target_fpath)
...
{{
        return MdUserDllWrapper.{spec_name}({argument_list});
}}'''
...
def _generate_api_function_dll_wrapper(spec):
    lines = []
    line1_fmt = '[DllImport("cctp_mduser.dll", CharSet = CharSet.Auto, EntryPoint = "%s",'
...
def _generate_spi_register_dll_wrapper(spec):
    line1_fmt = '[DllImport("cctp_mduser.dll", CharSet = CharSet.Auto, EntryPoint = "RegisterMdFP%s",'
